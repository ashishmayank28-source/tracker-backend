// frontend/src/pages/TeamAssignmentTable.jsx - Team Assignment Table for BM/Manager
...
export default function TeamAssignmentTable() {
  ...
  useEffect(() => {
    async function fetchTeam() {
      try {
        ...
        console.log("📋 Team members fetched:", filtered.length);
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Team Assignment Table");

    const dateStr = new Date().toISOString().slice(0, 10);
    XLSX.writeFile(wb, `Team_Assignment_Table_${user?.branch || "All"}_${dateStr}.xlsx`);
  };
  ...
  return (
    <div style={{ padding: 10 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <div>
          <h3 style={{ margin: 0, color: "#1e40af" }}>📊 Team Assignment Table</h3>
          <p style={{ margin: "5px 0 0", color: "#64748b", fontSize: 13 }}>
            {user?.role === "BranchManager" || user?.role === "Branch Manager" 
