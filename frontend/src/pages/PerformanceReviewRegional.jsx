...
export default function PerformanceReviewRegional() {
  ...
  const filteredEmployees = employees.filter(emp => {
    if (!["Employee", "Manager"].includes(emp.role)) return false;
  ...
  const branchSummary = branches.map(branch => {
    const branchEmps = employees.filter(e => e.branch === branch && ["Employee", "Manager"].includes(e.role));
    return {
      name: branch,
      empCount: branchEmps.length,
      totalTarget: branchEmps.reduce((sum, e) => sum + (e.target || 0), 0),
    };
  });
  ...
  return (
    <div style={{ padding: 20 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, flexWrap: "wrap", gap: 10 }}>
        <h2 style={{ margin: 0, fontSize: 22, fontWeight: 700 }}>⭐ Regional Performance Review</h2>
        <button
      ...
      <div style={{ 
        ...
        <div style={{ fontSize: 14, opacity: 0.9 }}>Region</div>
        <div style={{ fontSize: 24, fontWeight: 700 }}>{user?.region || "N/A"}</div>
        <div style={{ marginTop: 10, display: "flex", gap: 30 }}>
          <div>
            <div style={{ fontSize: 12, opacity: 0.8 }}>Total Employees</div>
            <div style={{ fontSize: 20, fontWeight: 600 }}>{filteredEmployees.length}</div>
          </div>
          <div>
            <div style={{ fontSize: 12, opacity: 0.8 }}>Branches</div>
            <div style={{ fontSize: 20, fontWeight: 600 }}>{branches.length}</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 15, marginBottom: 25, alignItems: "center", flexWrap: "wrap" }}>
        <div>
          <label style={{ fontSize: 12, color: "#6b7280", display: "block", marginBottom: 4 }}>Branch</label>
          <select
      ...
      {/* Branch-wise Summary (when viewing all) */}
      {selectedBranch === "all" && branches.length > 0 && (
        <div style={{ marginBottom: 30 }}>
          <h3 style={{ marginBottom: 15, fontSize: 18 }}>🏢 Branch-wise Overview</h3>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 15 }}>
            {branchSummary.map(branch => (
              ...
              >
                ...
                <div style={{ fontSize: 18, fontWeight: 700, color: "#166534", marginBottom: 12 }}>{branch.name}</div>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13 }}>
                  <span>👥 Employees</span>
                  <span style={{ fontWeight: 600 }}>{branch.empCount}</span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, marginTop: 4 }}>
                  <span>🎯 Total Target</span>
                  <span style={{ fontWeight: 600 }}>₹{branch.totalTarget.toLocaleString()}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Employee Cards */}
      <h3 style={{ marginBottom: 15, fontSize: 18 }}>
        👥 {selectedBranch === "all" ? "All Employees" : `${selectedBranch} Employees`}
      </h3>
      ...
      <div style={modalContent} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={modalHeader}>
          <div>
            ...
            <p style={{ margin: "4px 0 0 0", opacity: 0.9 }}>{emp.empCode} • {emp.branch} • {emp.area || "N/A"}</p>
          </div>
